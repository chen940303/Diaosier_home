...
class APITestCase(unittest.TestCase):
    ...
    def get_api_header(self, username, password):
        return {'Authorization':'Basic ' + b64encode((username + ':' + password).encode('utf-8')).decode('utf-8'),'Accept': 'application/json','Content-Type': 'application/json'}
    ...
    # 添加一个用户
        ...
        db.session.commit()
        # 写一篇文章
        response = self.client.post(url_for('api.new_post'),headers=self.get_api_header('john@example.com', 'cat'),data=json.dumps({'body': 'body of the *blog* post'}))
